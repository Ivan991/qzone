...
public partial class Blogs_ECategory : System.Web.UI.Page
{
            ...
            if (Session["id"] != null)
            {
                if (Request.QueryString["id"] != null && Request.QueryString["id"] == Session["id"].ToString() && r.IsMatch(Request.QueryString["id"]))
                {
...

                    {
                        ...
                        string sql1 = "select * from BlogCategory where OwnId=@id";//找出分类
    ...
    protected void btnadd_Click(object sender, EventArgs e)
    {
        ...
        {
            string sql = "select * from BlogCategory where OwnId=@ownid and Name=@name";
            ...
            if(dt.Rows.Count==0)
            {
                string sql1 = "insert into BlogCategory(Name,OwnId) values (@name,@ownid)";
                ...
                if(result>0)
                    ...
                    Response.Write("<script>alert('添加分类成功');location='/Blogs/Blog.aspx?id="+Request.QueryString["id"]+"'</script>");
    ...
    protected void category_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        if(e.CommandName=="btneditor1")
        {
            ...
            if(txt.Length>0)
            {
                string sql = "update BlogCategory set Name=@name where Id=@id";
                ...
                if(result>0)
                    ...
                    Response.Write("<script>alert('修改成功');location='/Blogs/Blog.aspx?id="+Request.QueryString["id"]+"'</script>");
        ...
        {
            string sql1 = "delete from BlogCategory where Id=@id";
            ...
            if(result1>0)
                ...
                Response.Write("<script>alert('删除成功');location='/Blogs/Blog.aspx?id=" + Request.QueryString["id"] + "'</script>");
