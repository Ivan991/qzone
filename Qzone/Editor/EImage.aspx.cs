...
public partial class EImage : System.Web.UI.Page
{
    ...
    protected void btnupload_Click(object sender, EventArgs e)
    {
        ...
        if (fileOK && sqlh.FileValidation.IsAllowedExtension(upload, fe))  //判断文件类型是否为bmp/gif/jpg/png以及检测头文件
        {
            ...
            if(result>0)
                ...
                Response.Write("<script>alert('修改空间头像成功');location='/Qzone.aspx?id=" + Request.QueryString["id"] + "'</script>");

            else

                Response.Write("<script>alert('修改空间头像失败')</script>");
        }
